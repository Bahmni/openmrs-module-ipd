...
public class DrugOrderScheduleResponse {
    ...
    public static DrugOrderScheduleResponse createFrom(DrugOrderSchedule drugOrderSchedule){
        return DrugOrderScheduleResponse.builder().
                ...
                pendingSlotsAvailable(drugOrderSchedule.getSlots().stream().anyMatch(slot -> (LocalDateTime.now(ZoneId.systemDefault()).isAfter(slot.getStartDateTime()) && slot.getStatus().equals(Slot.SlotStatus.SCHEDULED)))).
